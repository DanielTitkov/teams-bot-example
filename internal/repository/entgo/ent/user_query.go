...
func (uq *UserQuery) QueryDialog() *DialogQuery {
	query := &DialogQuery{config: uq.config}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withDialog; query != nil {
		...
		query.withFKs = true
		query.Where(predicate.Dialog(func(s *sql.Selector) {
			s.Where(sql.InValues(user.DialogColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_dialog
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_dialog" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
